...
export default function FollowModal({
  ...
  const handleAddStatus = async () => {
    ...
    if (status === 'authenticated' && followId.current) {
      ...
      const success: any = await follow.add(
  ...
  return (
    <Transition appear show={showModal} as={Fragment}>
      <Dialog as="div" className="relative z-[999]" onClose={handleCloseModal}>
        ...
        <div className="fixed left-0 right-0 top-1/2 -translate-y-1/2">
              ...
              <Dialog.Panel className="h-full w-full transform overflow-x-hidden rounded-2xl  bg-background p-6 text-left align-middle text-white shadow-xl transition-all md:w-[70%] lg:max-h-[85vh]">
                ...
                >
                  <div className="max-h-1/2 flex items-center justify-center md:col-span-1 md:items-start">
                    <figure
                      style={{
                        backgroundImage: `url(${comic?.thumb_url})`,
                      }}
                  ...
                  <div className="flex flex-col justify-between md:col-span-2 md:mt-4 md:px-10 lg:px-4">
                    <div className="flex flex-col gap-4">
                      <h1 className="ld:px-0 line-clamp-1 text-center font-secondary text-3xl font-bold md:px-4 md:text-left md:text-4xl">
                        {comic?.name}
                      </h1>

                      <h2 className="ld:px-0 line-clamp-1 text-center font-secondary md:my-8 md:px-4 md:text-left md:text-2xl lg:text-3xl">
                        Danh sách:
                      </h2>
