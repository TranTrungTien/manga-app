...
function Comment({ comment }: CommentProps) {
  ...
  const animationParent = useAutoAnimate({}) as RefObject<HTMLDivElement>;
  ...
  return (
    <div id={comment._id} className="h-fit" ref={animationParent}>
      <div className="flex items-center justify-between">
        ...
        <If
          condition={status === 'authenticated' && userId === comment.owner._id}
        >
          <Then>
            <CommentOptions handleSelectOptions={handleSelectOptions} />
          </Then>
        </If>
      </div>

      <Switch>
        <Case condition={displayMode === 'show' || displayMode === 'remove'}>
          <p
      ...
      <div className="flex space-x-4">
        <button
          ...
          className="smooth-effect rounded-xl bg-black px-5 py-4 hover:bg-highlight"
        >
