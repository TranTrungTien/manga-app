...
function Reader({ sideSettingState, closeDesktopPanel }: Readonly<IProps>) {
  ...
  return (
    <div ref={readerPageRef} className="h-fit min-h-screen w-full bg-black">
      <div
        ...
        className={`fixed right-0 top-0 z-[888] min-h-[50px] px-24`}
      >
        <SettingsMode
          handleConfig={handleConfig}
          show={Boolean(settings?.show)}
          styles={`${
            shouldMount && settings?.show ? 'slideUpReturn' : 'slideUp'
          } `}
        />
      </div>
      ...
      {reader?.loading === undefined ||
        (reader?.loading && (
          <div className="rounded-x absolute-center w-full py-6 font-secondary text-4xl">
            <CircleIcon
        ...
        <div className="h-[200px] w-full overflow-hidden py-4">
          <div className="mx-auto flex h-full w-full flex-col space-y-4 md:w-1/2">
            <div className="flex h-full w-full gap-4 ">
              ...
              >
                <ArrowLeftIcon className="inline h-8 w-8" /> Chapter trước
              </button>
              <button
                onClick={handleNavigateChapter}
                data-id="next"
            ...
            <h1 className="px-2 py-4 text-center text-white/75">
              Mẹo: Bạn có thể double tap/click vào 2 cạnh của màn hình để chuyển
              chap ở bất cứ vị trí nào ở chế độ dọc!
            </h1>
