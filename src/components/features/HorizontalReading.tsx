...
function HorizontalReading({
  ...
  const handleNavigateHorizontalPage = (e: MouseEvent<HTMLDivElement>) => {
    ...
    switch (e.currentTarget.dataset.id) {
      case 'right':
        readerDom?.scrollBy(Number(imgSize?.width), 0);
        break;
      case 'left':
        readerDom?.scrollBy(-Number(imgSize?.width), 0);
        break;
  ...
  useEffect(() => {
    ...
    const observerMiddleElem = () => {
      ...
      const elem = document.elementFromPoint(
        Number(readerSize?.width) / 2,
        Number(readerSize?.height) / 2,
      );
  ...
  useEffect(() => {
    const refCurrentpage = document.querySelector(`#page-${currentPage + 1}`);
    ...
    >
      ...
      >
        ...
        <h2 className="my-4 text-center text-primary">
          Đối với <DevicePhoneMobileIcon className="inline-block h-8 w-8" /> /{' '}
        ...
        <ul className="mx-auto w-[80%] list-disc space-y-4 px-6">
          ...
          <li>
            Vuốt sang trái/phải hoặc 1 chạm vào (2) cạnh màn hình để tiến/lùi
            trang trên màn hình.
          </li>
        </ul>
        <h2 className="my-4 text-center text-primary">
          Đối với <ComputerDesktopIcon className="inline-block h-8 w-8" />
        </h2>
        <ul className="mx-auto w-[80%] list-disc space-y-4 px-6">
          <li>
            Click sang trang kế bên hoặc dùng phím
            <ArrowLeftIcon className="mx-2 inline-block h-8 w-8" />
            <ArrowRightIcon className="mx-2 inline-block h-8 w-8" /> để tiến/lùi
            trang.
          </li>
        </ul>
        ...
        <ul className="mx-auto w-[80%] list-disc space-y-4 px-6">
          <li>
            ...
            <span className="text-primary">không phải là manga</span> mọi người
            nên đọc chế độ <span className="text-primary">dọc</span> để có trải
            nghiệm tốt nhất.
          </li>
          <li>
            Tại sao? Vì (Manhua, Manhwa/Webtoon) có lối vẽ trang rất dài và
            trang này nối tiếp trang kia (Không có ngắt trang) nên đọc ngang sẽ
            không thoải mái.
          </li>
