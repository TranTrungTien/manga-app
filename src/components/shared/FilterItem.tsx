...
export default function FilterItem({
  ...
  useEffect(() => {
    ...
    for (const key in router.query) {
      ...
      const existQuery = options?.find((option) => option.value === value);
      if (existQuery) {
        setQueryValue((prevState) => [
          ...prevState,
          {
            value: existQuery.value,
            label: existQuery.label,
          },
        ]);
      }
    }
  ...
  return (
    ...
    >
      <h2 className="my-2 font-secondary text-white lg:text-3xl">{title}</h2>
      <Select
...
const MoreSelectedBadge = ({ items }) => {
  ...
  return (
    <p title={title} className="rounded-sm bg-highlight p-1 text-sm text-white">
      {label}
    </p>
  );
};
